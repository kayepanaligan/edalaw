...
export default function SessionMonitoring({ sessions }: Props) {
    const handleJoinSession = (session: Session) => {
        router.post(`/monitoring-officer/sessions/${session.id}/join`, {}, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Successfully joined session as silent supervisor');
                // Redirect to video supervision page
                router.visit(`/monitoring-officer/video-supervision/${session.id}`);
            },
            onError: () => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            size="sm"
                            variant="default"
                            onClick={() => router.visit(`/monitoring-officer/video-supervision/${session.id}`)}
                        >
