...
{
    /**
     * Show the visit session page (join video call). Visitor must have accepted terms and be within schedule.
     */
    public function show(Request $request, VisitSession $session): InertiaResponse|RedirectResponse
    {
        ...
        if (! $session->isWithinSchedule()) {
            return redirect()->back()->withErrors(['session' => 'Session is only available during the scheduled time window.']);
        }
        ...
        if (! ($result['success'] ?? false) || empty($result['token'])) {
            return redirect()->back()->withErrors(['session' => 'Unable to generate join token. Please try again.']);
        }
