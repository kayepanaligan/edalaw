...
{
    /**
     * Display the appeals oversight page.
     */
    public function index(): Response
    {
        ...
        return Inertia::render('Admin/AppealsOversight', [
            'appeals' => $appeals,
            'stats' => $stats,
        ]);
    }
    ...
    /**
     * Review an appeal (approve or reject) - Super Admin can override.
     */
    public function review(Request $request, Appeal $appeal): RedirectResponse
    {
        ...
        AuditLogService::logAppealAction(
            'appeal_reviewed',
            $appeal,
            "Super Admin {$request->status} appeal for {$appeal->appealable_type} #{$appeal->appealable_id}. Notes: ".substr($request->decision_notes, 0, 100),
            $request
        );
    ...
    /**
     * Update appeal status (for quick status updates).
     */
    public function updateStatus(Request $request, Appeal $appeal): RedirectResponse
    {
        $validator = Validator::make($request->all(), [
        ...
        AuditLogService::logAppealAction(
            ...
            $appeal,
            "Super Admin updated appeal status to {$request->status} for {$appeal->appealable_type} #{$appeal->appealable_id}.",
            $request
        );

        return redirect()->back()->with('success', "Appeal status updated to {$request->status} successfully.");
    }
